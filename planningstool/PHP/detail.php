<?php 
	...
	if ($_SERVER["REQUEST_METHOD"] == "POST"){
		...
		if($errors == 0){
			...
			$sql2 = "INSERT INTO Ingeplande_Games(GameId,Start,Uitlegger,Players) VALUES(:gameId, :Start, :uitlegger, :players)";
			$statement2 = $pdo->prepare($sql2);
			$statement2->execute(["gameId" => $gameId, "Start" => $tijd, "uitlegger" => $uitlegger, "players" => implode(", ", $spelers)]);
		}else{
			$Incorrect = true;
		}
	}
...
<body>
	...
	<?php if ($Correct == true) { ?>
		<p class="toegevoegt">Spel toegevoegt</p>
	<?php } ?>
	<?php if ($Incorrect == true) { ?>
		<p class="mislukt">Toevoeging mislukt</p>
	<?php } ?>
	<div class="block">
	...
	<div class="block">
		<form method="POST">
			<div class="inputBox">
				<p>Begin tijd</p>
				<input class="input" type="text" name="tijd" placeholder="00:00" value="<?php echo isset($_POST["tijd"]) ? $_POST["tijd"]  : "" ?>">
				<span class="error"><?php echo $ErrTijd;?></span>
			</div>
			<?php foreach ($players as $key => $value) { ?>
			<div class="inputBox">
				<p>Naam speler <?php echo $value ?></p>
				<input class="input" type="text" name="<?php echo $key ?>" value="<?php echo isset($_POST[$key]) ? $_POST[$key]  : "" ?>">
				<span class="error"><?php echo $Err[$key];?></span>
			</div>
			<?php };?>
			<div class="inputBox">
				<p>Uitlegger</p>
				<input class="input" type="text" name="uitlegger" placeholder="Naam uitlegger" value="<?php echo isset($_POST["uitlegger"]) ? $_POST["uitlegger"]  : "" ?>">
